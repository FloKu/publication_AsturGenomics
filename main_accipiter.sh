...
#*****************************
# version 09.05.25, 16.00
# Min
# IQ tree test ganz unten im Skript
...
# It would have been smart to inlcude one Nisus sample in the lab work, but now we are doing it bioinformatically.
# Steps necessary:
#   7.1) Download the data from repositories
...
#*****************************
# Two nisus genomes are available in the www: GenBank and SRA.
# In the following chapter, I downloaded both and compared them and then decided on the GenBank one to use for mapping

# Metadata: Nisus genome (GenBank)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# GCA_004320145.1
# Nisus_Genomes/GenBank/
# Nisus genome from Genbank. https://www.ncbi.nlm.nih.gov/data-hub/genome/GCA_004320145.1/
# Generated by shotgun sequencing
# not assembled, looks like reads
cat /media/inter/fkunz/2022_Accipiter/data/Nisus_Genomes/GenBank/GCA_004320145.1/GCA_004320145.1_Accipiter_nisus_ver1.0_genomic.fna | head -2
grep -Inr ">" /media/inter/fkunz/2022_Accipiter/data/Nisus_Genomes/GenBank/GCA_004320145.1/GCA_004320145.1_Accipiter_nisus_ver1.0_genomic.fna | head -41
...
# 7.2) Mapping nisus genome (GenBank) to accipter genome (GenBank) 
#*****************************
# This code takes the sequence from the GenBank, not the reads from the SRA.
# Maps them to the reference genome (the same as above in chapter 4).
#   4 mappings are done: using NGM and BWA algorithms on both GCF and GCA references. 
# Then sorts it.

# We then use the CF reference, BWA-mapped files for further analyses!

# 7.2.1) **NOT USED** Mapping nisus genome (GenBank) to accipter genome (GenBank) using NextGenMap, GCF genome
#*****************************
echo '''
#!/bin/sh # the shebang - needed so that the sh is called (the hash in the shebang is part of the command)
...
''' > /media/inter/fkunz/2022_Accipiter/shell/5_NGMmapping_GCF.sh
qsub /media/inter/fkunz/2022_Accipiter/shell/5_NGMmapping_GCF.sh

# 7.2.2) **NOT USED** mapping nisus genome (GenBank) to accipter genome (GenBank) using NextGenMap, GCA genome
#*****************************
# this code takes the sequence from the GenBank, not the reads from the SRA, and maps them to the reference genome (the same as above in chapter 4)
# then sorts it
...
#*******************
ngm \
  -r /media/inter/fkunz/2022_Accipiter/data/Gentilis_reference/GCA_929447715.1/GCA_929447715.1_bAccGen1.1_alternate_haplotype_genomic.fna \
  -q /media/inter/fkunz/2022_Accipiter/data/Nisus_Genomes/GenBank/GCA_004320145.1/GCA_004320145.1_Accipiter_nisus_ver1.0_genomic.fna \
  -o /media/inter/fkunz/2022_Accipiter/results/mapped_nisus/nisus_NGMmapped_GCA.bam \
  -b
#  -t <int> # NUMBER OF THREADS USED FOR CANDIDATE SEARCH
#  -g <someting> # GPUS THAT SHULD BE USED

samtools sort /media/inter/fkunz/2022_Accipiter/results/mapped_nisus/nisus_NGMmapped_GCA.bam \
  -o /media/inter/fkunz/2022_Accipiter/results/mapped_nisus/nisus_NGMmapped_GCA_sorted.bam

cp /media/inter/fkunz/2022_Accipiter/results/mapped_nisus/nisus_NGMmapped_GCA_sorted.bam /media/inter/fkunz/2022_Accipiter/results/reference_aligned_GCA/Anis_NGM_GCA.bam
''' > /media/inter/fkunz/2022_Accipiter/shell/5_NGMmapping_GCA.sh
qsub /media/inter/fkunz/2022_Accipiter/shell/5_NGMmapping_GCA.sh

# 7.2.3) mapping nisus genome (SRA raw reads) to accipter genome (GenBank) using BWA, GCF genome
#*****************************
echo """
#!/bin/sh # the shebang - needed so that the sh is called (the hash in the shebang is part of the command)
...
# 7.2.4) **NOT IN USE** mapping nisus genome (SRA raw reads) to accipter genome (GenBank) using BWA, GCA genome
#*****************************
echo """
#!/bin/sh # the shebang - needed so that the sh is called (the hash in the shebang is part of the command)
...
#*******************
# run BWA to map, then SAMTOOLS to convert to bam file and sort it, save it in Nisus folder
bwa mem /media/inter/fkunz/2022_Accipiter/data/Gentilis_reference/GCA_929447715.1/GCA_929447715.1_bAccGen1.1_alternate_haplotype_genomic.fna \
/media/inter/fkunz/2022_Accipiter/data/Nisus_Genomes/SRA/downloaded_manually/SRR18767723_1.fastq \
/media/inter/fkunz/2022_Accipiter/data/Nisus_Genomes/SRA/downloaded_manually/SRR18767723_2.fastq \
| samtools view -bh \
| samtools sort > /media/inter/fkunz/2022_Accipiter/results/reference_aligned_GCA/Anis_BWA_GCA.bam
""" > /media/inter/fkunz/2022_Accipiter/shell/mapping/5_BWAmapping_GCA.sh
qsub /media/inter/fkunz/2022_Accipiter/shell/mapping/5_BWAmapping_GCA.sh
...
# 7.4) Subsetting the Nisus bam files using BED files
#*****************************
# This step is not needed anymore, as the mpileup doesnt need reduced files.

# This step was originally written as STACKS seems to have problems processing the raw reads given their sheer size, 
# given the Anis file contains the whole genome.
# Hence we are creating a BED file including all loci occuring in all samples to use that file to subsample the Anis BAM file.
# Individual bam files need to be indexed, as this was not done in the alignment step above. 
...
""" > /media/inter/fkunz/2022_Accipiter/shell/5_BEDfile_GCF.sh
qsub /media/inter/fkunz/2022_Accipiter/shell/5_BEDfile_GCF.sh

# Check Anis with FastQC - not needed
module load Tools/FastQC-0.11.9
cd /media/inter/fkunz/2022_Accipiter/results/reference_aligned_GCF
fastqc Anis.bam # change file name to somethin existing


# 7.5) MPILEUP stuff
#***********************************
# Goal: create Nisus that can be merged into the vcf file after populations, basically manually adding it
...
# Step 1: converting Anis_final_GCF.bam file to mpileup file or GCF <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Martin modified to use Anis_BWA_GCF.bam instead of Anis_final_GCF.bam for the generation of the Mpileup file
module load Tools/samtools-1.12
samtools mpileup \
  -B \
...
# Step 2: add mpileupAnis to the population_GCF file, liberal run
gzip /media/inter/fkunz/2022_Accipiter/results/populations_alldata_GCF_liberal/populations.snps.vcf
python /media/inter/fkunz/2022_Accipiter/scripts/Mpileup4VCF.py \
  --VCF /media/inter/fkunz/2022_Accipiter/results/populations_alldata_GCF_liberal/populations.snps.vcf.gz \
  --Mpileup /media/inter/fkunz/2022_Accipiter/results/nisus_pipeline/Anis_final_GCF.mp.gz \
  --output /media/inter/fkunz/2022_Accipiter/results/populations_alldata_GCF_liberal/final_data_GCF_lib.vcf.gz

# Step 2: add mpileupAnis to the population_GCF file, nodups run
...
# Step 2: add mpileupAnis to the population_GCF file, nowh146 run
...
# NOW FINALLY: we have vcf files now that include all accipiter individuals from STACKS pipeline and the Nisus genome from the Nisus-Pipeline. 
# The final vcf files need to be cleaned and can then be used for statistical analyses.

#---------------------------------------------------------------------------------------------------------
# Frage des Teufels: wäre es smart, die vcf files zuerst zu filtern und dann mit mpileup das anis dranzubinden?
#---------------------------------------------------------------------------------------------------------

# 7.6) Summary of results
#********************************************************************************************************************************************
# After a lot of different runs with different subsets and parameters, we now have:
# the NODPUS run, which includes gstacks and populations on all individuals (but no duplicates)
# the NOWH146 run, which indludes all samples except that one duplicate of 146 which is weird
...
python /media/inter/fkunz/2022_Accipiter/scripts/vcf2phylip_2.py \
  ...
  --output-folder /media/inter/fkunz/2022_Accipiter/results/FINAL_files \
  --output-prefix 21052024_63inds \
  -m 1 \
  -n
...
# 10) Clean vcf file using vcftools
#*****************************
# Here, we clean the final vcf file, as it has 40.000 loci and we might be able to reduce them 
# Two filtering steps are applied, one normal one and one conervative one
...
awk '$5 > 0.85' /media/inter/fkunz/2022_Accipiter/results/FINAL_files/out.imiss | cut -f1 > lowDP.indv

# convert into nexus
python /media/inter/fkunz/2022_Accipiter/scripts/vcf2phylip_2.py \
  -i //media/inter/fkunz/2022_Accipiter/results/FINAL_files/FINAL_63inds_cleaned.recode.vcf\
  --output-folder /media/inter/fkunz/2022_Accipiter/results/FINAL_files \
  --output-prefix FINAL_63inds_cleaned \
  -m 1 \
  -n
  
python /media/inter/fkunz/2022_Accipiter/scripts/vcf2phylip_2.py \
  -i /media/inter/fkunz/2022_Accipiter/results/FINAL_files/FINAL_63inds_cleaned_conservative.recode.vcf\
  --output-folder /media/inter/fkunz/2022_Accipiter/results/FINAL_files \
  --output-prefix FINAL_63inds_cleaned_conservative \
  -m 1 \
  -n

# convert uncleaned vcf file with 63 individuals into nexus file 6.05.2025
python /media/inter/fkunz/2022_Accipiter/scripts/vcf2phylip_2.py \
  -i //media/inter/fkunz/2022_Accipiter/results/populations_alldata_GCF_FINAL/FINAL_63inds.vcf\
...
#*******************
source /opt/anaconda3/etc/profile.d/conda.sh 
conda activate iq-tree-1.5.0

iqtree -s 21052024_63inds.min1.phy -m MF

''' > /media/inter/fkunz/2022_Accipiter/shell/11_IQtree_ModelFinder.sh
qsub /media/inter/fkunz/2022_Accipiter/shell/11_IQtree_ModelFinder.sh

# IQ tree starten
cd /media/inter/fkunz/2022_Accipiter/results
mkdir IQ-TREE

# load dependencies
module load Tools/iq-tree-1.5.0
...
iqtree -s /media/inter/fkunz/2022_Accipiter/results/populations_alldata_GCF_FINAL/FINAL_63inds.min1.phy \
